...
@pytest.mark.parametrize('medicine,replacements,expected', [
...
def test_script_generate_one_off_molecules(medicine : "list[str]", replacements : dict, expected : set):
    script = Script()
    actual = script.generate_one_off_molecules(medicine, replacements)
    assert actual == expected

@pytest.mark.parametrize('medicine,replacements,expected', [
    (['H','O','H'],{"H":["HO","OH"],"O":["HH"]},{"HOH","HOOH","HOHO","OHOH","HHHH","HOHHH","OHHHH","HHHHO","HHHOH","HOOHO","HOOOH","OHOHO","OHOOH","HOHHHO","HOHHOH","OHHHHO","OHHHOH"}),
])
def test_script_generate_all_molecules(medicine : "list[str]", replacements : dict, expected : set):
    molecules = set()
    script = Script()
    script.generate_all_molecules(medicine, replacements, molecules)
